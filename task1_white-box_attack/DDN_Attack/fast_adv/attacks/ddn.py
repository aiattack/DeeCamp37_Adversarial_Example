...
class DDN:
    ...
    def l2_norm1(self, input, axis = 1):
        norm = torch.norm(input, 2, axis, True)
        output = torch.div(input, norm)
        return output
    
    def attack(self, model: nn.Module, inputs: torch.Tensor, threshold: float, labels: torch.Tensor,
                  inputs_labels: torch.Tensor, targeted: bool = False) -> torch.Tensor:
        """
...
#         if inputs.min() < 0 or inputs.max() > 1: raise ValueError('Input values should be in the [0, 1] range.')
        ...
        for i in range(self.steps):
            ...
            adv = inputs + delta
            pred_labels = model(adv)
            ce_loss = 0 - self.l2_norm1(pred_labels).mm(labels.t())
            loss = multiplier * ce_loss
            pred_labelss = pred_labels.detach()
            judge_key = self.l2_norm1(pred_labelss).mm(labels.t())
            is_adv = (judge_key >= threshold) if targeted else (judge_key < 1-threshold)
            # is_adv = judge_key'''self.l2_norm1(pred_labels).mm(inputs_labels.t())'''
#             print('feature_similar:{}'.format(judge_key))
            is_adv = is_adv.squeeze(0)
